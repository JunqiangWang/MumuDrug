...
#' @export
PseudoSignatures_V0.1<-function(vpmat, method="maximalEffects"){
...
#' This function generate the combined pseudo signature from the drug perturbed signature
#' @param  vpmat The protein activity signature of the perturbed drugs
#' @return  The pseudo signatures of two combined drugs
#' @author Junqiang Wang
#' @export
PseudoSignatures<-function(vpmat, method="maximalEffects"){
  ...
  for (i in 1:ncol(vpmat)){
    ...
    for (j in i:ncol(vpmat)){
      ...
      tmp<-apply(tmp, 1, FUN = function(x){ifelse(abs(x[1])>abs(x[2]), x[1], x[2])})
